...
export const promptForInteractiveOptions = async ({
  ...
  const framework = await askChoice({
    question,
    output,
    prompt: 'Framework (express/node/hono) [express]: ',
    choices: {
      express: 'express',
      node: 'node',
      hono: 'hono',
    ...
    errorMessage: 'Please choose express, node, or hono.',
  });

  let typescript = false;
  if (framework === 'express') {
    typescript = await askChoice({
      question,
      output,
  ...
  let orm;
  if (framework === 'express') {
    const databaseChoices = typescript
      ? {
          none: false,
          no: false,
          n: false,
    ...
    const databasePrompt = typescript
      ? 'Database (none/mongodb/postgres) [none]: '
      : 'Database (none/mongodb) [none]: ';
    const databaseError = typescript
      ? 'Please choose none, mongodb, or postgres.'
      : 'Please choose none or mongodb.';
    ...
    if (db === 'postgres') {
      orm = await askChoice({
        question,
        output,
        prompt: 'ORM (prisma) [prisma]: ',
        choices: {
          prisma: 'prisma',
        },
        defaultValue: 'prisma',
        errorMessage: 'Please choose prisma.',
      });
    }
  }

  let view;
  if (framework === 'express' && db !== 'postgres') {
    view = await askChoice({
      question,
      output,
