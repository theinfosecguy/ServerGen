...
class AppGenerator {
   ...
   * @param {boolean} options.typescript - Whether to generate TypeScript project.
  ...
  /**
   * Gets the appropriate templates directory based on framework and typescript options.
   * @returns {string} Path to templates directory.
   */
  getTemplatesDir() {
    if (this.typescript) {
      return this.framework === 'node'
        ? this.config.paths.templates.nodeTs
        : this.config.paths.templates.expressTs;
    }
    return this.framework === 'node'
   ...
   */
  createAppStructure() {
    this.logger?.debug('Creating app structure', { framework: this.framework, typescript: this.typescript });
    
    if (this.typescript) {
      if (this.framework === 'node') {
        this.fileCreator.createNodeAppTypeScript(
          this.templatesDir,
          this.folderDir,
          this.appName,
          this.view,
        ...
        this.fileCreator.createExpressAppTypeScript(
          this.templatesDir,
          this.folderDir,
          this.appName,
          this.view,
  ...
  configurePort() {
    if (this.port !== 3000) {
      ...
      const indexPath = path.join(this.folderDir, `index.${ext}`);
      try {
  ...
  setupDatabase() {
    if (this.db) {
      if (this.typescript) {
        this.fileCreator.handleConfigTypeScript(
          this.folderDir,
          this.config.paths.templates.expressTs
        );
      } else {
