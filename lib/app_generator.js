...
class AppGenerator {
  ...
  _getTemplatesDir() {
    switch (this.framework) {
      ...
      case 'fastify':
        if (this.typescript) {
          return this.config.paths.templates['fastify-ts'];
        }
  ...
  createAppStructure() {
    ...
    if (this.framework === 'node') {
      ...
      if (this.typescript) {
        this.fileCreator.createFastifyTsApp(
          this.templatesDir,
          this.folderDir,
          this.appName,
          this.view,
  ...
  configurePort() {
    if (this.port !== 3000) {
      ...
      const indexFile = this.typescript ? 'src/index.ts' : 'index.js';
      const indexPath = path.join(this.folderDir, indexFile);
      try {
