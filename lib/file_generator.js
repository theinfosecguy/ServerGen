 ...
 * @param {string} framework - The framework type ('node', 'express', 'fastify', or 'fastify-ts').
 */
const generatePackage = (folderDir, appName, view, config, framework) => {
  const isTypescript = framework === 'fastify-ts';
  const pkg = {
    name: appName,
    version: '1.0.0',
    description: '',
  ...
  if (!isTypescript) {
    pkg.dependencies.nodemon = DEPENDENCY_VERSIONS.nodemon;
  }

  if (framework === 'express') {
    pkg.dependencies.express = DEPENDENCY_VERSIONS.express;
    pkg.dependencies.cors = DEPENDENCY_VERSIONS.cors;
  } else if (framework === 'fastify' || framework === 'fastify-ts') {
    pkg.dependencies.fastify = DEPENDENCY_VERSIONS.fastify;
    pkg.dependencies['@fastify/cors'] = DEPENDENCY_VERSIONS['@fastify/cors'];
    pkg.dependencies['@fastify/formbody'] = DEPENDENCY_VERSIONS['@fastify/formbody'];
    pkg.dependencies['@fastify/helmet'] = DEPENDENCY_VERSIONS['@fastify/helmet'];
    pkg.devDependencies['pino-pretty'] = '^13.0.0';
  ...
  if (view && VIEW_ENGINES[view]) {
    ...
    if (framework === 'fastify' || framework === 'fastify-ts') {
      pkg.dependencies['@fastify/view'] = DEPENDENCY_VERSIONS['@fastify/view'];
    }
  }
  ...
  if (config) {
    ...
    if (framework === 'fastify' || framework === 'fastify-ts') {
      pkg.dependencies['fastify-plugin'] = '^5.0.1';
    }
    if (isTypescript) {
...
/**
 * Creates a Fastify application with plugin-based structure.
 * @param {string} templatesDir - Path to Fastify templates.
 * @param {string} folderDir - The application directory path.
 * @param {string} appName - The application name.
 * @param {string|null} view - The view engine name.
 * @param {boolean} config - Whether to include mongoose configuration.
 */
const createFastifyApp = (templatesDir, folderDir, appName, view, config) => {
  const basePath = path.join(templatesDir, 'index.js');
  fs_helper.buildFilewithContents(basePath, folderDir, 'index.js');
  generateMVC(folderDir, templatesDir);
  generatePackage(folderDir, appName, view, config, 'fastify');
  console.log('Generating Fastify application..');
};

/**
 * Generates TypeScript MVC folder structure.
 * @param {string} folderDir - The application directory path.
 * @param {string} templatesDir - The templates directory path.
 */
const generateTsMVC = (folderDir, templatesDir) => {
  const dirs = ['src/controllers', 'src/model', 'src/routes'];
  ...
  const routesPath = path.join(templatesDir, 'src', 'routes', 'index.ts');
  if (fs.existsSync(routesPath)) {
    fs_helper.buildFilewithContents(
      routesPath,
      path.join(folderDir, 'src', 'routes'),
      'index.ts'
    );
  }
};
...
/**
 * Creates a Fastify TypeScript application.
 * @param {string} templatesDir - Path to Fastify TypeScript templates.
 * @param {string} folderDir - The application directory path.
 * @param {string} appName - The application name.
 * @param {string|null} view - The view engine name.
 * @param {boolean} config - Whether to include mongoose configuration.
 */
const createFastifyTsApp = (templatesDir, folderDir, appName, view, config) => {
  // Copy main index.ts
  const basePath = path.join(templatesDir, 'src', 'index.ts');
  fs.ensureDirSync(path.join(folderDir, 'src'));
  fs_helper.buildFilewithContents(basePath, path.join(folderDir, 'src'), 'index.ts');
  ...
  generateTsMVC(folderDir, templatesDir);
  generatePackage(folderDir, appName, view, config, 'fastify-ts');
  console.log('Generating Fastify TypeScript application..');
};
...
const manageViews = (folderPath, viewName, addView) => {
  // Check for TypeScript file first
  const tsFile = path.join(folderPath, 'src', 'index.ts');
  const jsFile = path.join(folderPath, 'index.js');
  const isTypeScript = fs.existsSync(tsFile);
  const targetFile = isTypeScript ? tsFile : jsFile;

  try {
    let data = fs.readFileSync(targetFile, 'utf8');
    const isFastify = data.includes('Fastify');

    if (addView) {
      let viewString;
      if (isFastify) {
        if (isTypeScript) {
          viewString = `import path from 'path';
import view from '@fastify/view';
fastify.register(view, {
  engine: { ${viewName}: require('${viewName}') },
  root: path.join(__dirname, '..', 'views'),
});`;
        } else {
          viewString = `const path = require('path');
fastify.register(require('@fastify/view'), {
  ...
  root: path.join(__dirname, 'views'),
        ...
        viewString = `app.set('view engine','${viewName}') \n app.set('views', path.join(__dirname, 'views'));`;
      }
 ...
 * @param {string} framework - The framework type ('node', 'express', or 'fastify').
 */
const handleConfig = (folderDir, templatesDir, framework) => {
  console.log('Configuring Mongoose..');

  // Check if TypeScript project
  const isTypeScript = fs.existsSync(path.join(folderDir, 'src'));

  if (isTypeScript) {
    fs_helper.createDir(path.join(folderDir, 'src'), 'config');
    const basePath = path.join(templatesDir, 'src', 'config', 'mongoose.ts');
    fs_helper.buildFilewithContents(
      basePath,
      path.join(folderDir, 'src', 'config'),
      'mongoose.ts'
    );
    const newIndexPath = path.join(templatesDir, 'src', 'index(config).ts');
    fs_helper.buildFilewithContents(newIndexPath, path.join(folderDir, 'src'), 'index.ts');
  } else {
