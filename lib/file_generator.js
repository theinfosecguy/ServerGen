...
const generatePackage = (folderDir, appName, view, config, framework, options = {}) => {
  ...
  const postgresPrisma = isPostgresPrisma(options.db, options.orm);
  const pkg = {
    name: appName,
    version: '1.0.0',
    description: '',
  ...
  if (postgresPrisma) {
    pkg.dependencies['@prisma/adapter-pg'] = DEPENDENCY_VERSIONS['@prisma/adapter-pg'];
    pkg.dependencies['@prisma/client'] = DEPENDENCY_VERSIONS['@prisma/client'];
    pkg.dependencies.pg = DEPENDENCY_VERSIONS.pg;
    pkg.devDependencies.prisma = DEPENDENCY_VERSIONS.prisma;
...
const createExpressApp = (templatesDir, folderDir, appName, view, config, options = {}) => {
  ...
  const indexTemplate = postgresPrisma
    ? `index(prisma).${generatedPaths.extension}`
    : config ? `index(config).${generatedPaths.extension}` : `index.${generatedPaths.extension}`;
  ...
  generateTypeScriptConfig(folderDir, appTemplatesDir, { postgresPrisma, typescript });
...
const generateTypeScriptConfig = (folderDir, templatesDir, options = {}) => {
  ...
  const tsconfigFile = options.postgresPrisma ? 'tsconfig.prisma.json' : 'tsconfig.json';
  const tsconfigPath = path.join(templatesDir, tsconfigFile);
...
/**
 * Adds Postgres + Prisma support files and mounts the generated users routes.
 * @param {string} folderDir - The application directory path.
 * @param {string} templatesDir - Path to Express templates.
 * @param {string} appName - The generated app name.
 */
const handlePostgresPrisma = (folderDir, templatesDir, appName) => {
  const templateRoot = path.join(templatesDir, 'typescript', 'postgres-prisma');
  const copyFile = (from, to) => {
    const sourcePath = path.join(templateRoot, from);
    const targetPath = path.join(folderDir, to);
  ...
  console.log('Configuring Postgres with Prisma..');
  copyFile('docker-compose.yml', 'docker-compose.yml');
  copyFile('prisma.config.ts', 'prisma.config.ts');
  copyFile(path.join('prisma', 'schema.prisma'), path.join('prisma', 'schema.prisma'));
  copyFile(path.join('prisma', 'seed.ts'), path.join('prisma', 'seed.ts'));
  copyFile(path.join('src', 'lib', 'prisma.ts'), path.join('src', 'lib', 'prisma.ts'));
  copyFile(
    path.join('src', 'controllers', 'usersController.ts'),
    path.join('src', 'controllers', 'usersController.ts')
  );
  copyFile(path.join('src', 'routes', 'users.ts'), path.join('src', 'routes', 'users.ts'));
  copyFile(path.join('test', 'users.test.ts'), path.join('test', 'users.test.ts'));

  const routesFile = path.join(folderDir, 'src', 'routes', 'index.ts');
  ...
  fs.writeFileSync(routesFile, routes, 'utf-8');

  const appTestFile = path.join(folderDir, 'test', 'app.test.ts');
  if (fs.existsSync(appTestFile)) {
    const appTest = fs
      .readFileSync(appTestFile, 'utf-8')
      .replace("from '../src/index';", "from '../src/index.js';");
    fs.writeFileSync(appTestFile, appTest, 'utf-8');
  }

  if (appName) {
    const schemaPath = path.join(folderDir, 'prisma', 'schema.prisma');
    const schema = fs
      .readFileSync(schemaPath, 'utf-8')
      .replace('ServerGen Postgres API', `${appName} API`);
...
const addDockerSupport = (folderDir, templatesDir, options = {}) => {
  const dockerfilePath = isPostgresPrisma(options.db, options.orm)
    ? path.join(templatesDir, 'typescript', 'postgres-prisma', 'Dockerfile')
    : options.typescript
...
`;
    ...
    if (isPostgresPrisma(options.db, options.orm)) {
      readme += `
## Postgres + Prisma

This app includes a Prisma 7 Postgres setup with a generated \`User\` model,
\`/users\` API routes, route tests, and a local Docker Compose database.

Start Postgres and prepare the database:
...
The generated user-route tests run when \`DATABASE_URL\` is configured.
Without a database URL, they are skipped while the basic app tests still run.
`;
    }
...
const addOpenApiSpec = (folderDir, options = {}) => {
  ...
  const hasUsersResource = isPostgresPrisma(options.db, options.orm);
...
`
    : '';
  const usersPaths = hasUsersResource
    ? `  /users:
    get:
      summary: List users
...
`
    : '';
  const usersSchemas = hasUsersResource
    ? `    User:
      type: object
      required:
        - id
...
${rootGetResponse}
        ...
        '500':
          $ref: '#/components/responses/InternalServerError'
${postRootPath}${honoInfoPaths}${usersPaths}  /health:
    get:
 ...
 * @param {false|'mongodb'|'postgres'|string} options.db - Database preset for generated environment values.
 * @param {number} options.port - Port configured for the generated app.
 */
const addEnvExample = (folderDir, templatesDir, options = {}) => {
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (
      options.db === false
      || (options.db && !isMongoDatabase(options.db) && !postgresPrisma)
    ) {
      env = env.replace(
        /# Database Configuration\nMONGODB_URI=.*\n\n/,
        ''
