...
describe('published package smoke test', () => {
  ...
  it(
    'generates, builds, tests, boots a Hono app, serves HTTP, and includes OpenAPI docs',
    async () => {
      const port = PORTS.hono;
      const appDir = generate('smokehono', 'hono', port, ['--openapi']);
      ...
      const spec = fs.readFileSync(path.join(appDir, 'docs', 'openapi.yaml'), 'utf-8');
      expect(spec).toContain("description: 'API specification for the smokehono Hono app generated by ServerGen.'");
      expect(spec).toContain('/about:');
      expect(spec).toContain('/contact:');
      expect(spec).toContain(`http://localhost:${port}`);
