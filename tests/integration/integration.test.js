...
describe('CLI Integration', () => {
  ...
  describe('Hono app generation', () => {
    it('generates a TypeScript Hono app with correct structure and scripts', () => {
      runCLI('-n honotest -f hono --skip-install');

      const appDir = path.join(testOutput, 'honotest');
      const pkg = JSON.parse(
        fs.readFileSync(path.join(appDir, 'package.json'), 'utf-8')
      );
      ...
      expect(fs.existsSync(path.join(appDir, '.dockerignore'))).toBe(true);
      expect(fs.existsSync(path.join(appDir, '.gitignore'))).toBe(true);
      expect(fs.existsSync(path.join(appDir, '.env.example'))).toBe(true);
      expect(fs.existsSync(path.join(appDir, 'index.js'))).toBe(false);
    ...
    it('generates JSON routes, import-safe startup, and custom support docs', () => {
      runCLI('-n honoport -f hono -p 8787 --skip-install');

      const appDir = path.join(testOutput, 'honoport');
      const index = fs.readFileSync(path.join(appDir, 'src', 'index.ts'), 'utf-8');
      const env = fs.readFileSync(path.join(appDir, '.env.example'), 'utf-8');
      const readme = fs.readFileSync(path.join(appDir, 'README.md'), 'utf-8');
      const dockerfile = fs.readFileSync(path.join(appDir, 'Dockerfile'), 'utf-8');
      const testFile = fs.readFileSync(
        path.join(appDir, 'test', 'app.test.ts'),
        'utf-8'
      );

      expect(index).toContain('process.env.PORT || 8787');
      ...
      expect(readme).toContain('# honoport');
      expect(readme).toContain('http://localhost:8787');
      expect(readme).toContain('npm test');
      expect(dockerfile).toContain('EXPOSE 8787');
      expect(testFile).toContain("app.request('/health')");
    });

    it('generates an OpenAPI spec for Hono when --openapi is used', () => {
      runCLI('-n honoopenapi -f hono --openapi -p 8788 --skip-install');

      const appDir = path.join(testOutput, 'honoopenapi');
      const specPath = path.join(appDir, 'docs', 'openapi.yaml');
      const readme = fs.readFileSync(path.join(appDir, 'README.md'), 'utf-8');
      const spec = fs.readFileSync(specPath, 'utf-8');

      expect(fs.existsSync(specPath)).toBe(true);
      expect(spec).toContain("title: 'honoopenapi API'");
      expect(spec).toContain('Hono app generated by ServerGen');
      expect(spec).toContain('url: http://localhost:8788');
      expect(spec).toContain('operationId: getRoot');
    ...
    it('accepts --typescript for Hono as a compatibility alias', () => {
      runCLI('-n honots -f hono --typescript --skip-install');

      expect(fs.existsSync(path.join(testOutput, 'honots', 'src', 'index.ts'))).toBe(true);
      expect(fs.existsSync(path.join(testOutput, 'honots', 'tsconfig.json'))).toBe(true);
    });
  });
  ...
  describe('unsupported flag combinations', () => {
    ...
    it('rejects --openapi with the node framework', () => {
      expectCLIError(
        ...
        'only supported with the express and hono frameworks'
      );
      expect(fs.existsSync(path.join(testOutput, 'nodeopenapi'))).toBe(false);
    });

    it('rejects --view with the hono framework', () => {
      expectCLIError(
        '-n honoview -f hono -v ejs --skip-install',
        'only supported with the express framework'
      );
      expect(fs.existsSync(path.join(testOutput, 'honoview'))).toBe(false);
    });
